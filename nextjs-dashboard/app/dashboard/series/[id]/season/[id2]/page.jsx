...
import { fetchSeasonById, saveSeasonById } from "@/app/lib/data";
...
export default function Page({ params }) {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        const data = await fetchSeasonById(params.id2);
        setFormData({...data });
        setLoading(false);
      } catch (error) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await saveSeasonById(params.id2, formData);
  ...
  return (
    <div className="max-w-md mx-auto p-6 bg-white rounded-lg shadow-md">
      <h1 className="text-2xl font-semibold text-gray-800 mb-4">Edit Season</h1>
